...
import com.netflix.clone.exception.EmailAlreadyExistException;
...
public class UserServiceImpl implements UserService {
    ...
    private void validateRole(String roleName) {
        if(Arrays.stream(Role.values()).noneMatch(r -> r.name().equalsIgnoreCase(roleName))) {
    ...
    private void ensureNotLastActiveAdmin(User user) {
        if(user.isActive() && user.getRole() == Role.ADMIN) {
            ...
            if(activeAdminCount <=1) {
                throw new RuntimeException("Cannot be deactivated the last admin user");
            }
        }
    }
    ...
    @Override
    public MessageResponse deleteUser(Long id, String curentUserEmail) {
        User user = serviceUtil.getUserByIdOrThrow(id);
        if(user.getEmail().equals(curentUserEmail)) {
    ...
    private void ensureNotLastAdmin(User user, String operation) {
        if(user.getRole() == Role.ADMIN) {
            ...
            if(adminCount <=1) {
                throw new RuntimeException("Cannot be " + operation + "the last admin user");
            }
        }
    }

    @Override
    public MessageResponse toggleUserStatus(Long id, String curentUserEmail) {
        User user = serviceUtil.getUserByIdOrThrow(id);

...
export class ManageVideo implements OnInit {
  ...
  static arrayNotEmpty(control: AbstractControl): Validators | null {
  ...
  onVideoPicked(ev: Event) {
    ...
    const hasValidExtension = validaVideoExtension.some((ext) => fileName.endsWith(ext));
    const hasValidMimeType =
      file.type.startsWith('video/') || file.type === 'application/octet-stream';

    if (!hasValidMimeType && !hasValidExtension) {
      this.notification.error('Please select a valid video file(MP$,MKV,etc.)');
      return;
    }
  ...
  removePoster() {
    ...
    this.posterPreviewUrl = null;
    this.videoForm.patchValue({ poster: '', duration: 0 });
    this.posterProgress = 0;
  }
}
